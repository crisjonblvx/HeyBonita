...
if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY) {
  console.error("[HBCU Deep Seeder] SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY missing.")
  process.exit(1)
}
...
const HBCU_CATEGORIES = [
  "HBCU_football",
  "Historically_black_college_and_university_marching_bands",
  "MEAC",
  "SWAC",
  "CIAA",
  "SIAC",
...
const TARGET_CATEGORY = "institution"

async function main() {
  console.log(`[HBCU Deep Seeder] Starting (${HBCU_CATEGORIES.length} categories) → ${TARGET_CATEGORY}`)
  let totalInserted = 0
  const seenTitles = new Set<string>()

  for (const categoryName of HBCU_CATEGORIES) {
    await sleep(300)
    ...
    console.log(`[HBCU Deep Seeder] ${categoryName} done.`)
  }

  const { count } = await supabase.from("knowledge_entries").select("id", { count: "exact", head: true }).eq("category", TARGET_CATEGORY)
  console.log(`[HBCU Deep Seeder] Done. New: ${totalInserted}. Total ${TARGET_CATEGORY}: ${typeof count === "number" ? count : "?"}`)
}
